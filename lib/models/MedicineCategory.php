...
class MedicineCategory {
  ...
  private $tbl = 'medicine_categories';
  ...
  public function getCategory($medicineID) {
    $query = QueryBuilder::rawQuery("SELECT `$this->tbl`.`name` AS 'name' FROM `medicines` INNER JOIN `$this->tbl` ON `medicines`.`medicine_category_id` = `$this->tbl`.`id` WHERE `medicines`.`id` = $medicineID");
  ...
  public function isDuplicate($name) {
    $query = QueryBuilder::select($this->tbl, [ 'name' ], "WHERE `name` = '$name'");
    $result = mysqli_query($GLOBALS['DB'], $query);
    $rows = mysqli_fetch_row($result);
    return $rows != null ? true : false;
  }

  public function isEditable($medicineID) {
    $query = QueryBuilder::rawQuery("SELECT * FROM `$this->tbl` INNER JOIN `medicines` ON `$this->tbl`.`id` = `medicines`.`medicine_category_id` WHERE `$this->tbl`.`id` = '$medicineID'");
    $result = mysqli_query($GLOBALS['DB'], $query);
    return mysqli_num_rows($result) == 0 ? true : false;
  }

  public function canBeDeleted($medicineID) {
    $query = QueryBuilder::rawQuery("SELECT * FROM `$this->tbl` INNER JOIN `medicines` ON `$this->tbl`.`id` = `medicines`.`medicine_category_id` WHERE `$this->tbl`.`id` = '$medicineID'");
    $result = mysqli_query($GLOBALS['DB'], $query);
    return mysqli_num_rows($result) == 0 ? true : false;
  }
